...
Let's get a count of the number of dogs that were reported to have been trained
with a reward-based program but were subject to a punishing restraining device.

```{r}
nrow(df %>%
  select(train_technique, punish_device) %>%
  drop_na() %>%
  filter(train_technique == 'reward' & punish_device == TRUE))
```
