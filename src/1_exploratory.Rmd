...
Now we can drop the column to simply the data set.

```{r}
df <- subset(df, select=-c(owner_id))
```

## Behavior Problem Indicator

Let's also add a column to signal if the dog has a behavior problem.
...
summary(df)
```
We also create pairwise scatter plots for all columns except for the individual
behavior problems (otherwise there would be too many plots.)

```{r, fig.height=5, fig.width=5}
df %>%
  na.omit %>%
  ggpairs(columns=c(1:6, 19:20, 30), progress=FALSE,
          ggplot2::aes(colour=behav_problem))
```

Observations on scatter plots:
