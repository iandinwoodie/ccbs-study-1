...
df %>%
  ggpairs(columns=c(1:4, 19), progress=FALSE, aes(color=behav_problem))
```
...
```{r}
ggplot(df, aes(age_yrs)) + geom_bar()
skewness(df$age_yrs)
```
...
ggplot(df, aes(log_age_yrs)) + geom_bar()
...
for (i in 1:length(outcomes)) {
  p <- ggplot(df, aes_string(x=outcomes[i], fill='train_age')) +
    geom_bar() +
    theme(legend.position="none")
  plot_list[[i]] <- p
}
